...
test("206 means ranges work", async () => {
  ...
  assert.deepEqual(result, { ok: true, ranges: true });
});

test("200 is usable but without ranges", async () => {
  const result = await probeSource("https://h/f.mkv", undefined, 100, respond({ status: 200 }));
  assert.deepEqual(result, { ok: true, ranges: false });
});
...
test("the probe asks for one byte and passes the stream's own headers", async () => {
  let seen;
  await probeSource("https://h/f.mkv", { Authorization: "Bearer x" }, 100, async (_url, init) => {
    seen = init.headers;
    return new Response(null, { status: 206 });
  });
  assert.equal(seen.Range, "bytes=0-0");
